...
- ✅ PDF出力＝印刷CSS方式（`SupportPrintSheet.tsx`。外部ライブラリ不要）。「シートに追加」で複数台を貯めて「PDFで見る」→プレビュー→印刷/PDFで保存。A4縦に1/2/4台を選べる。`SupportFigure` は `onEdit` を省くと静的SVGになるので、それをそのまま並べている
