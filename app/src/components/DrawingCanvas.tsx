...
interface Props {
  ...
  /**
   * 90°ショートエルボの「ショート」マークを表示するか。ショートを既定と
   * している会社では毎回全ての角に出てしまい図が読みにくいため、切れるように
   * している（表示専用。切り寸法の計算には影響しない）。
   */
...
export function DrawingCanvas({
  ...
  // 実際に描かれる45°/ショートマークの一覧（描画とレイアウトで同じものを使う）。
  // マークを持つセグメント自身だけでなく、ノードを共有する隣接セグメント側からも
  // 近さで判定するため、レンダー側(寸法線の実描画)でも同じ値を参照する。
  const markEnds = useMemo(
    () => elbowMarkEnds(segments, cutById, showShortMark),
    [segments, cutById, showShortMark],
  )
  const markKindByEnd = useMemo(() => elbowMarkKindByEnd(markEnds), [markEnds])
  const elbow45Marks = useMemo(() => markEnds.map((e) => e.pos), [markEnds])
  ...
  const resolvedLabels = useMemo(() => {
    ...
    // 45°(L45°/S45°)・ショートのエルボマークも、寸法ラベルが重なって文字が
    // 読めなくならないよう固定の回避領域として扱う（特に短いキック区間では
    // マークと寸法が近接しがち）。実際に描くマークと同じ位置を使う。
    const elbow45Obstacles: LabelBox[] = markEnds.map((e) => ({
      cx: e.pos.x,
      cy: e.pos.y,
      w: 34 * uiScale,
      h: 22 * uiScale,
  ...
  function flangeMarker(
    ...
    const nx = -uy
    const ny = ux
    // 端点の●や寸法線に埋もれて見落とされやすいため、線をやや長めに取る
    // （太さは .flange-mark 側で調整）。
    const half = 15
  ...
  // 45°(L45°/S45°)・90°ショート(ショート)のエルボマークを端に表示する。
  // ロングが既定のため、それ以外を使っている箇所を現場で見落とさないようにする。
  // どの端に何を描くかは markKindByEnd（エルボの角ごとに1つだけ選ばれている）で
  // 決まり、位置は markEnds と同じ計算を使う。
  function elbowMark(s: Segment, at: 'start' | 'end') {
    const kind = markKindByEnd.get(elbowMarkEndKey(s.id, at))
    if (!kind) return null
    const pt = at === 'start' ? s.start : s.end
    const other = at === 'start' ? s.end : s.start
    const len = distance(pt, other) || 1
    ...
    return (
      <text
        className={kind === 'short' ? 'elbow-short-mark' : 'elbow45-mark'}
        x={cx}
        y={cy}
        textAnchor="middle"
        dominantBaseline="middle"
      >
        {ELBOW_MARK_LABELS[kind]}
      </text>
    )
  }
        ...
        return (
          <g key={s.id} pointerEvents="none">
            ...
            {(cutById[s.id]?.endRole === 'tee-run-reducer' ||
              ...
              reducerAtEnd(s, 'end')}
            {/* 45°(L45°/S45°)・ショートのマーク。エルボの角は隣り合う2本の線が
                共有するため、角ごとに片方の線だけが描く（markKindByEnd で選択済み）。 */}
            {elbowMark(s, 'start')}
            {elbowMark(s, 'end')}
